...
wd <- "/Users/vkumar/Box Sync/LAB/PSY todo/Oral Meth Sucrose Methamphetamine Drinking KOMP/2024_Feb_JCMS_OM_Suc/Data/"

setwd(wd)
outputDir <- "../Suc_Processed/"

################# READ INPUT FILE 
filename <- "/Users/vkumar/Box Sync/LAB/PSY todo/Oral Meth Sucrose Methamphetamine Drinking KOMP/2024_Feb_JCMS_OM_Suc/Data/2024-02-09 TOM_MasterDataTableQuery v3.txt"
...
p1 <- ggplot(toPlot1, aes(x = DrugDose, y = DrugPreferenceScore, color = strainName, group = strainName)) +
      stat_summary(fun='mean', geom='point', position=pd) + 
      stat_summary(fun="mean", geom='line', position=pd) +
      geom_linerange(stat = "summary", fun.data = mean_se, position=pd) +
      graphTheme + 
      labs(title = toPlot, x= xaxisLabel, y = yaxisLabel) +
      theme(legend.position="top") +
      guides(col = guide_legend(nrow = 2)) +
